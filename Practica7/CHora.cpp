...
{
	AsignarFormato(ff);
	cout << "\n Constructor por omision invocado(CHora)." << endl;
}
/**/
...
CHora::CHora(const CHora& obj) : m_nHoras(obj.m_nHoras), m_nMinutos(obj.m_nMinutos), m_nSegundos(obj.m_nSegundos)
{
	...
	cout << "\n Constructor copia invocado(CHora)." << endl;
}
...
CHora& CHora::operator= (const CHora& obj_asg)
{
	...
	cout << "\n Operador de asignacion invocado(CHora)." << endl;
...
{
	Destruir();
	cout << "\n Destructor invocado(CHora)." << endl;
}